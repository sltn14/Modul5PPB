    ...
    <div className="min-h-screen bg-linear-to-br from-green-50 via-white to-cyan-50 pb-20 md:pb-8">
      <main className="max-w-7xl mx-auto px-4 md:px-8 py-8 md:py-12">
        <div className="mb-8 text-center">
          <h1 className="text-3xl md:text-5xl font-bold text-slate-800 mb-4">Resep Minuman</h1>
          <p className="text-slate-600 max-w-2xl mx-auto">Temukan berbagai resep minuman segar dan nikmat</p>
        </div>

        <AdvancedFilter onSearchChange={handleSearchChange} onFilterChange={handleFilterChange} initialFilters={{ ...filters, search: searchQuery }} />
          ...
          <>
                ...
                {filteredRecipes.map((r) => (
                  <article key={r.id} className="bg-white rounded-2xl shadow p-4 flex flex-col">
                    ...
                    <div className="mt-auto flex items-center gap-2">
                      <button onClick={() => onRecipeClick(r.id, 'minuman')} className="px-3 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">Detail</button>

                      <button onClick={() => handleShare(r)} className="px-3 py-2 bg-white border rounded-md hover:bg-slate-50" aria-label={`Bagikan ${r.name}`}>
            ...
            {pagination && pagination.total_pages > 1 && (
              <div className="mt-12 flex flex-col md:flex-row items-center justify-center gap-4">
                <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className="px-6 py-3 bg-white/80 backdrop-blur border border-slate-300 rounded-xl hover:bg-green-50 hover:border-green-400 disabled:opacity-50 disabled:cursor-not-allowed transition-all font-medium text-slate-700">← Sebelumnya</button>
                ...
                <button onClick={() => setPage(p => p + 1)} disabled={page === pagination.total_pages} className="px-6 py-3 bg-white/80 backdrop-blur border border-slate-300 rounded-xl hover:bg-green-50 hover:border-green-400 disabled:opacity-50 disabled:cursor-not-allowed transition-all font-medium text-slate-700">Selanjutnya →</button>
              </div>
            )}
          </>
        )}
